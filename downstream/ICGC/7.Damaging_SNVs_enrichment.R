...
for(marker in MARKERS){
  ...
  if(marker == "GRHL"){
    ...
    variants_anno$enh_anno <- "non-functional"
    variants_anno[variants_anno$name %in% anno_enhancers$name, ]$enh_anno <- "functional"
    variants_anno$enh_anno <- factor(variants_anno$enh_anno, levels = c("functional", "non-functional"))
    
    n_enh <- variants_anno %>% group_by(., enh_anno) %>% dplyr::summarise(., "n" = length(unique(name)))
    ...
    n_enh_1 <- variants_anno %>% dplyr::filter(., damaging == 1) %>% 
      group_by(., enh_anno) %>% dplyr::summarise(., "n" = length(unique(name)))
    n_enh_2 <- variants_anno %>% dplyr::filter(., damaging == 2) %>% 
      group_by(., enh_anno) %>% dplyr::summarise(., "n" = length(unique(name)))
    print("Fraction of enhancers with at least one damagin mutation - Level 1: ")
...
#' TODO: 
#' - Change enhancers contrasts (the idea is to use functional vs. non-functional enhancers), and script accordingly
