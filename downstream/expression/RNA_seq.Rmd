...
# Local 
IN_FOLDER <- fs::path("/Users/ieo6983/Desktop/expression/")
OUT_FOLDER_DEGs <- fs::path("/Users/ieo6983/Desktop/expression/DEGs_length_scaled/")

# HPC
#IN_FOLDER <- fs::path("/hpcnfs/scratch/PGP/ebugani/expression/results/")
#OUT_FOLDER_DEGs <- fs::path("/hpcnfs/scratch/PGP/Ciacci_et_al/results/expression/RNA_seq/DEGs/")
...
```{r}
# Input matrix gene_counts x sample - Length scaled 
g_1st <- read_tsv(fs::path(IN_FOLDER,"results_1st_group/star_salmon/salmon.merged.gene_counts_length_scaled.tsv"))
g_2nd <- read_tsv(fs::path(IN_FOLDER, "results_2nd_group/star_salmon/salmon.merged.gene_counts_length_scaled.tsv"))
...
#write.csv(df_export, file=fs::path(OUT_FOLDER_DEGs, "DEGs_Condition_GRHL2KD_SCR.csv"))
...
spec_genes <- c("GRHL2", "GRHL1", "CDH1", "CDH2", "SNAI1", "SNAI2", "ZEB1","TWIST1", "VIM", "RBBP8", "OVOL1", "OVOL2")
spec_genes <- spec_genes[spec_genes %in% df_LFC$gene_name]
...
#df_LFC_sig %>% arrange(., log2FoldChange) %>% write_tsv(., fs::path(OUT_FOLDER_DEGs, paste0("Df_DEGs.df_LFC_sig.padj_", significance_threshold, ".log2FC_1.Up_and_Down.tsv")))
...
## Enrichment analysis 
```{r}
# Save gene sets for DGE analysis
save_all <- F

if(save_all == T){
  data.frame(df_LFC_sig[df_LFC_sig$DE == "Up", ]$gene_id) %>% write_tsv(., fs::path(OUT_FOLDER_DEGs, paste0("DEGs.df_LFC_sig.padj_", significance_threshold, ".log2FC_1.UP.tsv")), col_names = F)
  data.frame(df_LFC_sig[df_LFC_sig$DE == "Up", ]$gene_name) %>% write_tsv(., fs::path(OUT_FOLDER_DEGs, paste0("DEGs.df_LFC_sig.padj_", significance_threshold, ".log2FC_1.UP.names.tsv")), col_names = F)
  
  data.frame(df_LFC_sig[df_LFC_sig$DE == "Down", ]$gene_id) %>% write_tsv(., fs::path(OUT_FOLDER_DEGs, paste0("DEGs.df_LFC_sig.padj_", significance_threshold, ".log2FC_1.DOWN.tsv")), col_names = F)
  data.frame(df_LFC_sig[df_LFC_sig$DE == "Down", ]$gene_name) %>% write_tsv(., fs::path(OUT_FOLDER_DEGs, paste0("DEGs.df_LFC_sig.padj_", significance_threshold, ".log2FC_1.DOWN.names.tsv")), col_names = F)

  # Not shrinked
  data.frame(df_export_sig[df_export_sig$DE == "Up", ]$gene_id) %>% write_tsv(., fs::path(OUT_FOLDER_DEGs, paste0("DEGs.df_export_sig.padj_", significance_threshold, ".log2FC_1.UP.tsv")), col_names = F)
  data.frame(df_export_sig[df_export_sig$DE == "Up", ]$gene_name) %>% write_tsv(., fs::path(OUT_FOLDER_DEGs, paste0("DEGs.df_export_sig.padj_", significance_threshold, ".log2FC_1.UP.names.tsv")), col_names = F)

  data.frame(df_export_sig[df_export_sig$DE == "Down", ]$gene_id) %>% write_tsv(., fs::path(OUT_FOLDER_DEGs, paste0("DEGs.df_export_sig.padj_", significance_threshold, ".log2FC_1.Down.tsv")), col_names = F)
  data.frame(df_export_sig[df_export_sig$DE == "Down", ]$gene_name) %>% write_tsv(., fs::path(OUT_FOLDER_DEGs, paste0("DEGs.df_export_sig.padj_", significance_threshold, ".log2FC_1.Down.names.tsv")), col_names = F)
...
Alternatively, DGE can be easily run from EnrichR web tool: https://maayanlab.cloud/Enrichr/. 
...
GSEA gene sets can be downloaded from: https://www.gsea-msigdb.org/gsea/msigdb/ 
```{r}
# Gene sets 
...
#ggsave(plot = p$Plot, filename = fs::path(OUT_FOLDER_DEGs, "GSEA/Plots/GSEA.DB_h.all.v2023.2.Hs.top_10.png"), device = "png", width = 10, height = 6)
```
