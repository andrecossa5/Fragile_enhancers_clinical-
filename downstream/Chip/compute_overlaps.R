...
path_peaks_union2 <- fs::path("/Users/ieo6983/Desktop/fragile_enhancer_clinical/data/functional_genomics/Chip/Chip_for_clusters/results/CtIP_GRHL/downstream/peaks_union/")
...
new1 <- list("CtIP" = read_tsv(fs::path(path_peaks_union1, "merged_peaks.hg19_CtIP.filtered_K27ac_k4me1_k4me3.hq_signal.new_summit.narrowPeak"), 
             ...
             "GRHL" = read_tsv(fs::path(path_peaks_union1, "merged_peaks.hg19_GRHL.filtered_K27ac_k4me1_k4me3.hq_signal.new_summit.narrowPeak"), 
                               col_names = narrowPeak_colnames_nostrand))
new2 <- list("CtIP" = read_tsv(fs::path(path_peaks_union2, "merged_peaks.hg19_CtIP.filtered_K27ac_k4me1_k4me3.new_summit.narrowPeak"), 
                              col_names = narrowPeak_colnames_nostrand),
            "GRHL" = read_tsv(fs::path(path_peaks_union2, "merged_peaks.hg19_GRHL.filtered_K27ac_k4me1_k4me3.new_summit.narrowPeak"), 
                              col_names = narrowPeak_colnames_nostrand))
...
for(marker in MARKERS){
  ...
  new2_gr <- new2[[marker]] %>% makeGRangesFromDataFrame(., keep.extra.columns=T)
  seqlevels(new2_gr) <- paste0("chr", seqlevels(new2_gr))
  new2_gr_all[[marker]] <- new2_gr
  ...
  ovrlps2 <- !is.na(new2_gr %>% findOverlaps(., old_gr, select = "arbitrary"))
  ovrlps2v <- !is.na(old_gr %>% findOverlaps(., new2_gr, select = "arbitrary"))
  ...
  print(paste0("Percentage of ", marker, " peaks p01 overlapping with old ones - ", 
               sum(ovrlps2), " over ", length(new2_gr), " : ~", (sum(ovrlps2) / length(new2_gr)) %>% round(.,2) *100, "%"))
  print(paste0("Viceversa - old overlaps with new2 - : ", sum(ovrlps2v), " over ", length(old_gr), " : ", (sum(ovrlps2v) / length(old_gr)) %>% round(.,2) *100, "%"))
}
...
# new2 - p-value < 0.01
forw <- !is.na(findOverlaps(new2_gr_all$CtIP, new2_gr_all$GRHL, select = "arbitrary"))
backw <- !is.na(findOverlaps(new2_gr_all$GRHL, new2_gr_all$CtIP, select = "arbitrary"))
print(paste0("NEW2 CtIP enhancers overlapping with GRHL2 enhancers: ", 
             sum(forw), " over ", length(forw), " - ", (sum(forw) / length(forw)) %>% round(.,2)*100, "%" ))
print(paste0("NEW2 GRHL enhancers overlapping with CtIP enhancers: ", 
             sum(backw), " over ", length(backw), " - ", (sum(backw) / length(backw)) %>% round(.,2)*100, "%" ))
